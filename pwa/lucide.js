...
// Auto-init disabled — controlled by app.js _refreshIcons() to prevent re-processing already-replaced SVGs
// (iOS Safari throws InvalidCharacterError when createIcons() finds already-replaced SVGs and reads their path d-values as attribute names)
// lucide.createIcons();
...
(function (global, factory) {
    ...
    const createSVGElement = ([tag, attrs, children]) => {
        ...
        Object.keys(attrs).forEach((name) => {
            // Guard against invalid attribute names (e.g. SVG path values mistaken for attr names in iOS Safari)
            try { element.setAttribute(name, String(attrs[name])); } catch (_lucideErr) {}
        });
        if (children?.length) {
